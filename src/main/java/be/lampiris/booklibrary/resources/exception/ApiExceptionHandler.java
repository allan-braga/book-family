package be.lampiris.booklibrary.resources.exception;
...
public class ApiExceptionHandler extends ResponseEntityExceptionHandler {
	...
	protected ResponseEntity<Object> handleMethodArgumentNotValid(MethodArgumentNotValidException ex,
			HttpHeaders headers, HttpStatus status, WebRequest request) {
		List<Error> errorsList = generateErrorList(ex.getBindingResult());
		return handleExceptionInternal(ex, errorsList, headers, HttpStatus.BAD_REQUEST, request);
	}
	...
	public ResponseEntity<Object> handleDataIntegrityViolationException(DataIntegrityViolationException ex, WebRequest request){
		...
		List<Error> errorsList = Arrays.asList(new Error(userMessage, developerMessage));
		return handleExceptionInternal(ex, errorsList, new HttpHeaders(), HttpStatus.BAD_REQUEST, request);
	}
	...
	private List<Error> generateErrorList(BindingResult bindingResult){
		...
		for(FieldError fieldError : bindingResult.getFieldErrors()) {
			...
			errorsList.add(new Error(userMessage, developerMessage));
		}
		return errorsList;
	}
